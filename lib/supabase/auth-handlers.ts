...
/**
 * Extract error from URL hash or query params
 * Handles Supabase error responses in both #error=... and ?error=... formats
 */
export function getAuthError(): {
  ...
  // Try hash first (client-side auth flows)
  ...
  // Try query params (server-side flows)
  const params = new URLSearchParams(window.location.search);
  const qError = params.get("error");
  const qDescription = params.get("error_description");
  const qCode = params.get("error_code");

  if (qError) {
    return {
      error: qError,
      description: qDescription ? decodeURIComponent(qDescription) : null,
      code: qCode,
    };
  }

  return { error: null, description: null, code: null };
}

/**
 * Get user-friendly error message from error code
 */
export function getErrorMessage(error: string, code?: string | null, description?: string | null): string {
  // Use description if available (most accurate)
  if (description) {
    return description;
  }
  ...
  switch (code) {
    ...
    case "invalid_otp":
      return "Invalid link. Please check the URL and try again.";
    case "user_not_found":
