...
import { linkUserToHRM } from "../_actions/users";

type User = { id: string; fullName: string; email: string };

export default function AddHRMUserDialog({
  onUserAdded,
}: {
  onUserAdded: () => void;
}) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await linkUserToHRM({
      ...
      if (result.error) {
        ...
        toast.success("User added to HRM");
        setDialogOpen(false);
        setFormData({ userId: "", role: "EMPLOYEE" });
        onUserAdded();
      }
    });
  };
  ...
  return (
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Add User to HRM</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label>
              HRM Role <span className="text-red-500">*</span>
            </Label>
            <Select
              value={formData.role}
              onValueChange={(value) =>
                setFormData({
                  ...formData,
                  role: value as "SUPER_ADMIN" | "ADMIN" | "EMPLOYEE",
                })
              }
            >
              <SelectTrigger>
